...
module rv32i_regfile (
  ...
  // RF-005: asynchronous read.
  // The CPU drives write data from combinational execute logic, so bypassing
  // wdata_i here would feed ALU result back into its own operands when an
  // instruction reads and writes the same register, e.g. ADDI x2, x2, imm.
  always_comb begin
